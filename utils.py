...
def get_start(segments: List[dict]) -> Optional[float]:
    return next((w["start"] for s in segments for w in s["words"]), segments[0]["start"] if segments else None)
...
class SubtitlesWriter(ResultWriter):
    ...
    def iterate_result(self, result: dict, options: Optional[dict] = None, *, max_line_width: Optional[int] = None, max_line_count: Optional[int] = None, highlight_words: bool = False, max_words_per_line: Optional[int] = None):
        options = options or {}
            ...
            for subtitle in iterate_subtitles():
                ...
                if highlight_words:
                    ...
                    for i, this_word in enumerate(subtitle):
                        ...
                        yield start, end, "".join([re.sub(r"^(\s*)(.*)$", r"\1<u>\2</u>", word) if j == i else word for j, word in enumerate(all_words)])
                        last = end
                else:
